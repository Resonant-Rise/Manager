...
?&#62;';
		...
		if(!empty($_POST['move_perm']) && is_logged_in() && is_admin()) {
			...
			if($_POST['action'] == "up") {
				...
				$get = mysqli_query($con,"SELECT * FROM permissions WHERE id='$id'");
				...
				if(mysqli_num_rows($get) > 0) {
					...
					$get_next = mysqli_query($con,"SELECT * FROM permissions WHERE level='$next'");
					...
					if($g['name'] == "Admin") {
						...
						if(mysqli_num_rows($get_next) > 0) {
							...
							if($g_next['name'] == "Admin") {
								...
								mysqli_query($con,"UPDATE permissions SET level='$next' WHERE id='$id'");
								mysqli_query($con,"UPDATE permissions SET level='$level' WHERE id='$nextid'");
							}
						} else {
				...
				if(mysqli_num_rows($get) > 0) {
					...
					$get_previous = mysqli_query($con,"SELECT * FROM permissions WHERE level='$previous'");
					...
					if($g['name'] == "Admin") {
						...
						if(mysqli_num_rows($get_previous) > 0) {
							...
							if($level < 2 || $previous < 1) {
								...
								mysqli_query($con,"UPDATE permissions SET level='$previous' WHERE id='$id'");
								mysqli_query($con,"UPDATE permissions SET level='$level' WHERE id='$previousid'");
							}
						} else {
							// Check if permission level isn't going to be lower than 1
							if($level < 2) {
		...
		if(!empty($_POST['create_perm']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['perm_name'])) {
				...
				$get_admin = mysqli_query($con,"SELECT * FROM permissions WHERE name='Admin'");
				...
				for($i=$adminlevel; $i>$level; $i--) {
					$newlevel = $i + 1;
					mysqli_query($con,"UPDATE permissions SET level='$newlevel' WHERE level='$i'");
				}

				$newlevel = $level + 1;

				mysqli_query($con,"INSERT INTO permissions(name, level, on_login, on_logout, no_permission)
		...
		if(!empty($_POST['save_perm']) && is_logged_in() && is_admin()) {
			...
			$check = mysqli_query($con,"SELECT * FROM permissions WHERE id='$id'");
			...
			if(mysqli_num_rows($check) == 1) {
				...
				if(empty($_POST['perm_name']) && $c['name'] != "Admin") {
					...
					if($c['name'] == "Admin") {
						mysqli_query($con,"UPDATE permissions SET on_login='$on_login', on_logout='$on_logout', no_permission='$no_permission' WHERE id='$id'");
						echo 1; // Echo 1, JavaScript will handle the rest
					} else {
						mysqli_query($con,"UPDATE permissions SET name='$name', on_login='$on_login', on_logout='$on_logout', no_permission='$no_permission' WHERE id='$id'");
						echo 1; // Echo 1, JavaScript will handle the rest
					}
				}
			} else {
		...
		if(!empty($_POST['delete_perm']) && is_logged_in() && is_admin()) {
			...
			if($g['name'] == "Admin") {
				...
				mysqli_query($con,"UPDATE users SET permission='$default' WHERE permission='$id'"); // Update all users from this permission to the default permission

				mysqli_query($con,"DELETE FROM permissions WHERE id='$id'");
				echo 1; // Echo 1, JavaScript will handle the rest
			}
		}
		...
		if(!empty($_POST['move_user']) && is_logged_in() && is_admin()) {
			...
			$users = mysqli_query($con,"SELECT * FROM users WHERE permission='$pid'");
			...
			if(mysqli_num_rows($users) > 0) {
				while($u = mysqli_fetch_array($users)) {
					$userid = $u['id'];
					// If the user id is posted, and the username is not admin and the permission of the user is not the same the user will be moved to the new permission
					if(!empty($_POST[$userid]) && $u['username'] != "admin" && $u['permission'] != $perm) {
						mysqli_query($con,"UPDATE users SET permission='$perm' WHERE id='$userid'");
				...
				if($count == 0) {
					...
					$permission = mysqli_query($con,"SELECT * FROM permissions WHERE id='$perm'");
		...
		if(!empty($_POST['add_input']) && is_logged_in() && is_admin()) {
			...
			if(empty($input_name)) {
				...
				$input_check = mysqli_query($con,"SELECT * FROM inputs WHERE name='$input_name'");
				...
				if(mysqli_num_rows($input_check) > 0) {
					...
					$get = mysqli_query($con,"SELECT * FROM inputs ORDER BY place DESC LIMIT 1");
					$g = mysqli_fetch_array($get);
					...
					mysqli_query($con,"INSERT INTO inputs (place, name, type, placeholder, value, required, input_error, maxlength, min, max, step, rows, checked, options, public)
					VALUES ('$newplace','$input_name','$input_type','$input_placeholder','$input_value','$input_required','$input_error','$input_maxlength','$input_min','$input_max','$input_step','$input_rows','$input_checked','$options','$input_public')");

					mysqli_query($con,"ALTER TABLE users ADD ". $input_name ." TEXT NOT NULL"); // Create column in user table
		...
		if(!empty($_POST['move_input']) && is_logged_in() && is_admin()) {
			...
			if($_POST['action'] == "up") {
				$id = htmlentities(mysqli_real_escape_string($con, $_POST['id']), ENT_QUOTES);
				$get = mysqli_query($con,"SELECT * FROM inputs WHERE id='$id'");
				...
				if(mysqli_num_rows($get) > 0) {
					...
					$get_next = mysqli_query($con,"SELECT * FROM inputs WHERE place='$next'");
					...
					if(mysqli_num_rows($get_next) > 0) {
						...
						mysqli_query($con,"UPDATE inputs SET place='$next' WHERE id='$id'");
						mysqli_query($con,"UPDATE inputs SET place='$place' WHERE id='$nextid'");
					} else {
						echo "<div class='alert alert-danger' role='alert'><a href='#' class='close' data-dismiss='alert'>&times;</a>". $m['not_higher'] ."</div>";
					}
				} else {
				...
				if(mysqli_num_rows($get) > 0) {
					...
					$get_previous = mysqli_query($con,"SELECT * FROM inputs WHERE place='$previous'");
					...
					if(mysqli_num_rows($get_previous) > 0) {
						...
						if($place < 2 || $previous < 1) {
							...
							mysqli_query($con,"UPDATE inputs SET place='$previous' WHERE id='$id'");
							mysqli_query($con,"UPDATE inputs SET place='$place' WHERE id='$previousid'");
						}
					} else {
		...
		if(!empty($_POST['save_input']) && is_logged_in() && is_admin()) {
			...
			$check = mysqli_query($con,"SELECT * FROM inputs WHERE id='$id'");
			...
			if(mysqli_num_rows($check) == 1) {
				...
				if(empty($_POST['input_name'])) {
					...
					if(mysqli_num_rows($input_check) > 0 && $c['name'] != $input_name) {
						...
						mysqli_query($con,"ALTER TABLE users CHANGE ". $c['name'] ." ". $input_name ." TEXT NOT NULL"); // Change column in user table
						mysqli_query($con,"UPDATE inputs SET name='$input_name', type='$type', placeholder='$placeholder', required='$required', input_error='$input_error', value='$input_value', maxlength='$input_maxlength', min='$input_min', max='$input_max', step='$input_step', rows='$input_rows', checked='$checked', options='$options', public='$input_public' WHERE id='$id'");
						echo 1; // Echo 1, JavaScript will handle the rest
					}
				}
			} else {
		...
		if(!empty($_POST['delete_input']) && is_logged_in() && is_admin()) {
			...
			$last_place = mysqli_query($con,"SELECT * FROM inputs ORDER BY place DESC LIMIT 1");
			...
			for($i=$place + 1; $i<=$highest_place; $i++) {
				$newplace = $i - 1;
				mysqli_query($con,"UPDATE inputs SET place='$newplace' WHERE place='$i'");
			}

			mysqli_query($con,"ALTER TABLE users DROP COLUMN ". $g['name']); // Delete column in user table
			mysqli_query($con,"DELETE FROM inputs WHERE id='$id'"); // Delete input
		...
		if(!empty($_POST['adminprofile']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['username'])) {
				...
				if(userValue($uid, "username") != $username && mysqli_num_rows($check_username) > 0) {
					...
					$inputs = mysqli_query($con,"SELECT * FROM inputs");
					$errors = 0;
					...
					if($errors == 0) {
						...
						while($i2 = mysqli_fetch_array($inputs2)) {
							...
							if(isset($_POST[$name]) || $i2['type'] == "checkbox" && empty($_POST[$name])) {
								...
								mysqli_query($con,"UPDATE users SET ". $name ."='$option' WHERE id='$uid'");
							}
						}

						// If the username is not admin, update default inputs
						if(userValue($uid, "username") != "admin") {
							mysqli_query($con,"UPDATE users SET username='$username', email='$email', permission='$permission' WHERE id='$uid'");
						}
		...
		if(!empty($_POST['adminchangepass']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['newpass'])) {
				...
				mysqli_query($con,"UPDATE users SET password='$newpassword' WHERE id='$uid'"); // Update password
			...
			if(empty($_POST['username']) && getSetting("username_change", "text") == "true") {
				...
				if(getSetting("username_change", "text") == "true" && userValue($uid, "username") != $username && mysqli_num_rows($check_username) > 0) {
					...
					if($errors == 0) {
						...
						// If it is allowed to change a username, and the username is not admin, update it
						if(getSetting("username_change", "text") == "true" && userValue($uid, "username") != "admin") {
							mysqli_query($con,"UPDATE users SET username='$username' WHERE id='$uid'");
						}
		...
		if(!empty($_POST['changepass']) && is_logged_in() && getSetting("password_change", "text") == "true" && userValue(null, "password") != "") {
			...
			if(empty($_POST['oldpass'])) {
				...
				$check = mysqli_query($con,"SELECT * FROM users WHERE id='$uid' AND password='$oldpassword'");
				...
				if(mysqli_num_rows($check) == 0) {
					...
					mysqli_query($con,"UPDATE users SET password='$newpassword' WHERE id='$uid' AND password='$oldpassword'"); // Update new password
		...
		if(!empty($_POST['setpass']) && is_logged_in() && userValue($_SESSION['uid'], "password") == "") {
			...
			if(empty($_POST['newpass'])) {
				...
				$uid = $_SESSION['uid'];
				mysqli_query($con,"UPDATE users SET password='$newpassword' WHERE id='$uid'"); // Update new password
		...
		if(!empty($_POST['add_block']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['ip'])) {
				...
				$check = mysqli_query($con,"SELECT * FROM blocks WHERE ip='$ip'");
				// Check if IP is already blocked
				if(mysqli_num_rows($check) == 0) {
					mysqli_query($con,"INSERT INTO blocks(time, ip, logs, reason, until) VALUES('$time','$ip','$logs','$reason','$until')"); // Add block
		...
		if(!empty($_POST['delete_block']) && is_logged_in() && is_admin()) {
			...
			$blocks = mysqli_query($con,"SELECT * FROM blocks");
				...
				if(!empty($_POST[$id])) {
					...
					while($l = mysqli_fetch_array($logs)) {
						...
						if($l['success'] == "1") {
							...
							mysqli_query($con,"DELETE FROM log WHERE id='$lid'"); // Delete failed login
						}
					}

					mysqli_query($con,"DELETE FROM blocks WHERE id='$id'"); // Delete IP block
		...
		if(!empty($_POST['change_permission']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['permission'])) {
				...
				while($u = mysqli_fetch_array($users)) {
					$uid = $u['id'];
					// Check if user id is posted, and if the username is not admin and the permission is not the same
					if(!empty($_POST[$uid]) && $u['username'] != "admin" && $u['permission'] != $permission) {
						mysqli_query($con,"UPDATE users SET permission='$permission' WHERE id='$uid'"); // Update user with new permission
				...
				if($count == 0) {
					...
					$perm = mysqli_query($con,"SELECT * FROM permissions WHERE id='$permission'");
		...
		if(!empty($_POST['ban_user']) && is_logged_in() && is_admin()) {
			...
			while($u = mysqli_fetch_array($users)) {
				$uid = $u['id'];
				$check = mysqli_query($con,"SELECT * FROM bans WHERE uid='$uid'");
				// Check if the user id is posted, and the username is not admin and the user isn't already banned
				if(!empty($_POST[$uid]) && $u['username'] != "admin" && mysqli_num_rows($check) == 0) {
					$time = time();

					mysqli_query($con,"INSERT INTO bans (time, uid) VALUES ('$time', '$uid')"); // Ban user
		...
		if(!empty($_POST['unban_user']) && is_logged_in() && is_admin()) {
			...
			while($u = mysqli_fetch_array($users)) {
				$uid = $u['id'];
				$check = mysqli_query($con,"SELECT * FROM bans WHERE uid='$uid'");
				// Check if the user id is posted, and the username is not admin and the user is banned
				if(!empty($_POST[$uid]) && $u['username'] != "admin" && mysqli_num_rows($check) != 0) {
					$time = time();

					mysqli_query($con,"DELETE FROM bans WHERE uid='$uid'"); // Unban user
		...
		if(!empty($_POST['activate_user']) && is_logged_in() && is_admin()) {
			...
			while($u = mysqli_fetch_array($users)) {
				$uid = $u['id'];
				// Check if the user id is posted, and the username is not admin and the user isn't already active
				if(!empty($_POST[$uid]) && $u['username'] != "admin" && $u['active'] != "1") {
					$time = time();

					mysqli_query($con,"UPDATE users SET active='1' WHERE id='$uid'"); // Activate user
		...
		if(!empty($_POST['deactivate_user']) && is_logged_in() && is_admin()) {
			...
			while($u = mysqli_fetch_array($users)) {
				$uid = $u['id'];
				// Check if the user id is posted, and the username is not admin and the user is active
				if(!empty($_POST[$uid]) && $u['username'] != "admin" && $u['active'] != "0") {
					$time = time();

					mysqli_query($con,"UPDATE users SET active='0' WHERE id='$uid'"); // Deactivate user
		...
		if(!empty($_POST['delete_user']) && is_logged_in() && is_admin()) {
			...
			while($u = mysqli_fetch_array($users)) {
				$uid = $u['id'];
				// Check if the user id is posted, and the username is not admin
				if(!empty($_POST[$uid]) && $u['username'] != "admin") {
					$time = time();

					mysqli_query($con,"DELETE FROM users WHERE id='$uid'"); // Delete user
					$deleted++; // Add 1 to deleted
				}
			}
		...
		if(!empty($_POST['mass_message']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['sendto'])) {
				...
				if($sendto == "everyone") {
					...
					while($u = mysqli_fetch_array($users)) {
						$to = $u['id'];
						mysqli_query($con,"INSERT INTO messages(sendfrom, sendto, subject, message, time, opened) VALUES ('$from','$to','$subject','$message','$time','0')"); // Create message
					}
				} else {
					$users = mysqli_query($con,"SELECT * FROM users WHERE permission='$sendto'");
					$total = mysqli_num_rows($users);
		...
		if(!empty($_POST['create_backup']) && is_logged_in() && is_admin()) {
			...
			echo $m['save_backup'];
			echo "<pre>". htmlentities($return, ENT_QUOTES) ."</pre>"; // Show backup code, this code should be saved to a .sql file or just filled in in a SQL query
		}
		...
		if(!empty($_POST['open_messages']) && is_logged_in()) {
			...
			while($msg = mysqli_fetch_array($messages)) {
				...
				if(!empty($_POST[$mid]) && $msg['sendto'] == $uid && $msg['opened'] != "1") {
					mysqli_query($con,"UPDATE messages SET opened='1' WHERE id='$mid' AND sendto='$uid'"); // Open message
		...
		if(!empty($_POST['delete_messages']) && is_logged_in()) {
			...
			while($msg = mysqli_fetch_array($messages)) {
				...
				if(!empty($_POST[$mid]) && $msg['sendto'] == $uid) {
					mysqli_query($con,"DELETE FROM messages WHERE id='$mid' AND sendto='$uid'"); // Delete message
		...
		if(!empty($_POST['send_message']) && is_logged_in()) {
			...
			if(empty($_POST['sendto'])) {
				...
				$check = mysqli_query($con,"SELECT * FROM messages WHERE sendfrom='$from' ORDER BY id DESC LIMIT 1");
		...
		if(!empty($_POST['count_items']) && is_logged_in() && is_admin()) {
			...
			if($item == "messages") {
				$get = mysqli_query($con,"SELECT * FROM messages WHERE time < ". $from);
				...
				$get = mysqli_query($con,"SELECT * FROM log WHERE time < ". $from);
				$count = mysqli_num_rows($get);
				...
				if($based == "activity") {
					$get = mysqli_query($con,"SELECT * FROM users WHERE last_login < ". $from ." AND last_login != '' AND username != 'admin'");
				} else {
					$get = mysqli_query($con,"SELECT * FROM users WHERE registered_on < ". $from ." AND username != 'admin'");
				}
				...
				$get = mysqli_query($con,"SELECT * FROM users WHERE active='0'");
				$count = mysqli_num_rows($get);
				...
				$get = mysqli_query($con,"SELECT * FROM users WHERE last_login=''");
				$count = mysqli_num_rows($get);
		...
		if(!empty($_POST['clean_messages']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['from'])) {
				...
				$check = mysqli_query($con,"SELECT * FROM users WHERE id='$uid' AND password='$password'");
				...
				if(mysqli_num_rows($check) != "1") {
					...
					$messages = mysqli_query($con,"SELECT * FROM messages WHERE time < ". $from); // Select everything older than the given timestamp
					while($msg = mysqli_fetch_array($messages)) {
						$id = $msg['id'];
						mysqli_query($con,"DELETE FROM messages WHERE id='$id'"); // Delete message
		...
		if(!empty($_POST['clean_logs']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['from'])) {
				...
				if(mysqli_num_rows($check) != "1") {
					...
					$logs = mysqli_query($con,"SELECT * FROM log WHERE time < ". $from); // Select everything older than the given timestamp
					while($l = mysqli_fetch_array($logs)) {
						$id = $l['id'];
						mysqli_query($con,"DELETE FROM log WHERE id='$id'"); // Delete log
		...
		if(!empty($_POST['clean_users']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['from'])) {
				...
				if(mysqli_num_rows($check) != "1") {
					...
					if($based == "activity") {
						$users = mysqli_query($con,"SELECT * FROM users WHERE last_login < ". $from ." AND last_login != ''"); // Select everything older than the given timestamp and where the user has already logged in
						while($u = mysqli_fetch_array($users)) {
							// Make sure that the username is not admin
							if($u['username'] != "admin") {
								$id = $u['id'];
								mysqli_query($con,"DELETE FROM users WHERE id='$id'"); // Delete user
								$deleted++; // Add 1 to deleted to show how many users are deleted
							}
						}
					} else {
						$users = mysqli_query($con,"SELECT * FROM users WHERE registered_on < ". $from); // Select everything older than the given timestamp
						while($u = mysqli_fetch_array($users)) {
							// Make sure that the username is not admin
							if($u['username'] != "admin") {
								$id = $u['id'];
		...
		if(!empty($_POST['delete_inactive']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['password'])) {
				...
				if(mysqli_num_rows($check) != "1") {
					...
					mysqli_query($con,"DELETE FROM users WHERE active='0'"); // Delete if user is not active
		...
		if(!empty($_POST['delete_never_loggedin']) && is_logged_in() && is_admin()) {
			...
			if(empty($_POST['password'])) {
				...
				if(mysqli_num_rows($check) != "1") {
					...
					mysqli_query($con,"DELETE FROM users WHERE last_login=''"); // Delete all users where last login is empty
			...
			if(empty($_FILES["file"]["type"])) {
				...
				&& in_array($extension, $extensions))) {
					...
					if ($_FILES["file"]["error"] > 0) {
						...
						$check = mysqli_query($con,"SELECT avatar FROM users WHERE id='$uid'");
						$c = mysqli_fetch_array($check);
						...
						if(file_get_contents($_FILES['file']['tmp_name']) != file_get_contents("../uploads/". $c['avatar'])) {
							...
							mysqli_query($con,"UPDATE users SET avatar='$newname' WHERE id='$uid'"); // Update the users avatar with only the new file name
		...
		if(!empty($_POST['forgot_pass']) && !is_logged_in()) {
			...
			if(empty($_POST['username'])) {
				...
				if(mysqli_num_rows($check) == 1 && $username != "admin") {
					$u = mysqli_fetch_array($check);

					$ip = $_SERVER['REMOTE_ADDR'];
					$code_check = mysqli_query($con,"SELECT * FROM forgot_codes WHERE ip='$ip' ORDER BY id DESC LIMIT 1"); // Get last forgotten password request from IP
					...
					if(mysqli_num_rows($code_check) == 0 || mysqli_num_rows($code_check) == 1 && $newtime < time()) {
						...
						mysqli_query($con,"INSERT INTO forgot_codes (time, uid, used, code, ip)
		...
		if(!empty($_POST['forgot_changepass']) && !is_logged_in()) {
			...
			if(empty($_POST['password'])) {
				...
				$check_code = mysqli_query($con,"SELECT * FROM forgot_codes WHERE code='$code'");
				...
				if(mysqli_num_rows($check_code) == 0) {
					...
					if(strtotime("+6 hours", $cc['time']) > time()) {
						...
						if($cc['used'] == "0") {
							...
							mysqli_query($con,"UPDATE users SET password='$password' WHERE id='$uid'"); // Update user with new password
							mysqli_query($con,"UPDATE forgot_codes SET used='1' WHERE code='$code'"); // Update code to used
		...
		if(!empty($_POST['register'])) {
			...
			if(!is_logged_in()) {
				...
				if(getSetting("max_ip", "text") > 0) {
					$checkip = mysqli_query($con,"SELECT * FROM users WHERE ip='$ip'");
						...
						if(getSetting("disable_register", "text") == "false") {
							...
							if($errors == 0) {
								...
								if((getSetting("enable_paypal", "text") == "true" && getSetting("paypal_cost", "text") != "0" && getSetting("enable_stripe", "text") != "true") || $method == "1") {
									...
									$getuid = mysqli_query($con,"SELECT id FROM users WHERE username='$username'");
									...
									mysqli_query($con,"UPDATE users SET paypal='$querystring' WHERE username='$username'"); // Update user with paypal query string for if the user cancels his payment to retry
								...
								while($i2 = mysqli_fetch_array($inputs2)) {
									...
									if(!empty($_POST[$name]) || $i2['type'] == "checkbox" && empty($_POST[$name])) {
										...
										mysqli_query($con,"UPDATE users SET ". $name ."='$option' WHERE username='$username'");
									}
								}
					...
					if(mysqli_num_rows($check_username) > 0) {
						...
						if(getSetting("disable_register", "text") == "false") {
							...
							if($errors == 0) {
								...
								if($active == 1) {
									mysqli_query($con,"UPDATE users SET last_login='$registered_on' WHERE username='$username'"); // Update last login

									$get_uid = mysqli_query($con,"SELECT id FROM users WHERE username='$username'");
						...
						if(mysqli_num_rows($checkblock) > 0 && $cb['until'] < $timenow && !empty($cb['until']) && $cb['until'] != "0") {
							...
							while($l = mysqli_fetch_array($logs)) {
								...
								if($l['success'] == "1") {
									...
									mysqli_query($con,"DELETE FROM log WHERE id='$lid'"); // Delete failed log
								}
							}

							$bid = $cb['id'];
							mysqli_query($con,"DELETE FROM blocks WHERE id='$bid'"); // Delete IP block
						...
						if(mysqli_num_rows($checkblock) > 0 && ($cb['until'] > $timenow || empty($cb['until']) || $cb['until'] == "0") && $unblocked == 0) {
							...
							if(getSetting("login_with", "text") == "email") {
								...
								if(getSetting("case_sensitive", "text") == "true") {
									$check = mysqli_query($con,"SELECT * FROM users WHERE BINARY email='$loginname' AND password='$pass'");
								} else {
									$check = mysqli_query($con,"SELECT * FROM users WHERE email='$loginname' AND password='$pass'");
								}
								...
								// Check if the login is case sensitive
								if(getSetting("case_sensitive", "text") == "true") {
									$check = mysqli_query($con,"SELECT * FROM users WHERE BINARY username='$loginname' AND password='$pass'");
								} else {
									$check = mysqli_query($con,"SELECT * FROM users WHERE username='$loginname' AND password='$pass'");
								}

								$getuid = mysqli_query($con,"SELECT * FROM users WHERE username='$loginname'");
							}
							...
							if(mysqli_num_rows($check) == 0) {
								...
								if(mysqli_num_rows($bancheck) > 0) {
									...
									mysqli_query($con,"UPDATE users SET last_login='$last_login' WHERE username='$uname'"); // Update last login
									...
									$perm = $c['permission'];
									$getperm = mysqli_query($con,"SELECT * FROM permissions WHERE id='$perm'");
