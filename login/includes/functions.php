...
function get($value, $table) {
	...
	$get = mysqli_query($con,"SELECT * FROM ". $table); // Select from given table
...
function setting($setting, $value) {
	...
	$check = mysqli_query($con,"SELECT * FROM settings WHERE setting='$setting'");

	// Check if setting already exists, if so update it, else create it
	if(mysqli_num_rows($check) > 0) {
		mysqli_query($con,"UPDATE settings SET value='$value' WHERE setting='$setting'");
	} else {
		mysqli_query($con,"INSERT INTO settings (setting, value) VALUES ('$setting','$value')");
	}
}
...
function getSetting($setting, $type) {
	...
	$getsetting = mysqli_query($con,"SELECT * FROM settings WHERE setting='$setting'");
	...
	if(mysqli_num_rows($getsetting) == 1) {
		...
		if($type == "option") {
			if(!empty($gs['value'])) {
				$id = $gs['value'];
				$getperm = mysqli_query($con,"SELECT name FROM permissions WHERE id='$id'");
...
function getExtraInputs($filled = false, $uid = null) {
	...
	$inputs = mysqli_query($con,"SELECT * FROM inputs ORDER BY place DESC");
		...
		if($i['type'] == "textarea") {
			...
			<div class='row row-1'>
				<div class='form-group'>
					<label class='col-sm-4 control-label'><?php echo $i['name']; ?><?php if($i['required'] == "true") { echo "*"; } ?></label>
					<div class='col-sm-8'>
...
function addLog($success, $ip, $uid = 0, $try, $type) {
	...
	if($uid == 0) {
		mysqli_query($con,"INSERT INTO log (success, time, ip, try, type)
		VALUES ('$success','$time','$ip','$try','$type')");
	} else {
		mysqli_query($con,"INSERT INTO log (success, time, ip, uid, try, type)
...
function getPermId($uid) {
	...
	$guser = mysqli_query($con,"SELECT * FROM users WHERE id='$uid'");
...
function getTypeUrl($type, $uid = null) {
	...
	if($type == "notify") {
		...
		if(isset($_SERVER['HTTPS'])) {
			return "https://". $matches[0] . $script_path ."callback/paypal.php";
		} else {
			return "http://". $matches[0] . $script_path ."callback/paypal.php";
		}
	} elseif($type == "return") {
		// Check if SSL is enabled
		if(isset($_SERVER['HTTPS'])) {
			return "https://". $matches[0] . $script_path ."login.php?m=4";
		} else {
			return "http://". $matches[0] . $script_path ."login.php?m=4";
		}
	} elseif($type == "cancel") {
		// Check if SSL is enabled
		if(isset($_SERVER['HTTPS'])) {
			return "https://". $matches[0] . $script_path ."login.php?m=5";
		} else {
			return "http://". $matches[0] . $script_path ."login.php?m=5";
		}
	} elseif($type == "reset") {
		// Check if SSL is enabled
		if(isset($_SERVER['HTTPS'])) {
			return "https://". $matches[0] . $script_path ."login.php?forgot&code=";
		} else {
			return "http://". $matches[0] . $script_path ."login.php?forgot&code=";
		}
	} elseif($type == "activation") {
		// Check if SSL is enabled
		if(isset($_SERVER['HTTPS'])) {
			return "https://". $matches[0] . $script_path ."login.php?activation&activate_code=";
		} else {
			return "http://". $matches[0] . $script_path ."login.php?activation&activate_code=";
		}
	} elseif($type == "google_redirect") {
		// Check if SSL is enabled
		if(isset($_SERVER['HTTPS'])) {
			return "https://". $matches[0] . $script_path ."social.php?return=google";
		} else {
			return "http://". $matches[0] . $script_path ."social.php?return=google";
		}
	} elseif($type == "twitter_callback") {
		// Check if SSL is enabled
		if(isset($_SERVER['HTTPS'])) {
			return "https://". $matches[0] . $script_path ."social.php?return=twitter";
		} else {
			return "http://". $matches[0] . $script_path ."social.php?return=twitter";
		}
...
function isAdminByNameOrEmail($logginginwith) {
	...
	$check1 = mysqli_query($con,"SELECT * FROM users WHERE username='$logginginwith' AND permission='1'"); // Check if the given input is a username
	$check2 = mysqli_query($con,"SELECT * FROM users WHERE email='$logginginwith' AND permission='1'"); // Check if the given input is a email address
...
function isAdminBySid($sid) {
	...
	$check = mysqli_query($con,"SELECT * FROM users WHERE sid='$sid' AND permission='1'"); // Check if the given input is a username
...
function logout() {
	...
	$permid = $u['permission'];
	$perm = mysqli_query($con,"SELECT * FROM permissions WHERE id='$permid'");
...
function activateCode($activate_code) {
	...
	if(mysqli_num_rows($check) == 1) {
		...
		if($u['active'] == "0") {
			mysqli_query($con,"UPDATE users SET active='1' WHERE id='$uid'"); // Activate the user
