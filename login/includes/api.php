...
if(!isset($con)) {
	require('/Users/admin/Public/mods/login/config.php');
}
if(!isset($titles)) {
	require('/Users/admin/Public/mods/login/lang/english.php');
}
...
function logged_in() {
	...
	if(empty($_SESSION['uid']) || empty($_SESSION['ip']) || $_SESSION['ip'] != $_SERVER['REMOTE_ADDR'] || checkUid($_SESSION['uid']) == false) {
		setcookie("last_url", getCurrentUrl()); // Save last url in a cookie
...
function allowedLevels($levels) {
	...
	$level = explode(",", $levels); // Separate the levels from the commas
...
function rankUp($uid = null, $levels = 1, $savemode = true) {
	...
	if(mysqli_num_rows($user) > 0) {
		...
		if($p['name'] == "Admin" && $savemode == true) {
			...
			$newperm = mysqli_query($con,"SELECT * FROM permissions WHERE level='$newlevel'");
			...
			if(mysqli_num_rows($newperm) > 0) {
				...
				if($np['name'] == "Admin" && $savemode == true) {
					...
					// Rankup successful
					$newid = $np['id'];
					mysqli_query($con,"UPDATE users SET permission='$newid' WHERE id='$uid'");
...
function checkLogin($type = "username", $input, $password) {
	...
	if($type == "email") {
		$check = mysqli_query($con,"SELECT * FROM users WHERE email='$input' AND password='$password'");
	} else {
		$check = mysqli_query($con,"SELECT * FROM users WHERE username='$input' AND password='$password'");
	}
