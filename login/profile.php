...
if(!empty($_GET['id'])) {
	...
	$user = mysqli_query($con,"SELECT * FROM users WHERE username='$u'");
}
			...
			<table class='table borderless'>
				<thead>
					...
					<tr>
						...
						<td><?php echo date("j F Y", $u['last_login']) ." ". $m['at'] ." ". date("G:i", $u['last_login']); ?></td>
					</tr>
					...
					<?php
					$extra_inputs = mysqli_query($con,"SELECT * FROM inputs WHERE public='true' ORDER BY place DESC"); // Get extra inputs
	...
	<div role='tabpanel'>
		<div class='row'>
			<ul class='nav nav-tabs' role='tablist' id='tabs'>
				...
				$messages = mysqli_query($con,"SELECT * FROM messages WHERE sendto='$uid' AND opened='0'");
				?>
			...
			<div role='tabpanel' class='tab-pane' id='messages'>
				...
				?>
					...
					?>
						...
						$message = mysqli_query($con,"SELECT * FROM messages WHERE sendto='$uid' AND id='$mid'");
						...
						if($msg['opened'] == "0") {
							mysqli_query($con,"UPDATE messages SET opened='1' WHERE sendto='$uid' AND id='$mid'");
						}
						?>
					...
					?>
						...
						<?php
						$message = mysqli_query($con,"SELECT * FROM messages WHERE sendto='$uid' ORDER BY id DESC");
						...
						<div class='row row-0'>
							<div class='form-group' id='actions' style='margin-bottom: -34px;'>
								<button type='button' class='btn btn-primary' onclick='openMessages("<?php echo session_id(); ?>");' style='position: relative; top: -60px;'><?php echo $m['mark_opened']; ?></button>
			...
			<div class='modal-body'>
				...
				<form method='post' id='send_message_form'>
					<div class='form-group'>
						...
						<select class='selectpicker' name='sendto[]' data-live-search='true' multiple>
							<?php
							$users = mysqli_query($con,"SELECT * FROM users ORDER BY username");
