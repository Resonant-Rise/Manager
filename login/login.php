...
<div class='container-small'>
	...
	if(mysqli_num_rows($checkblock) > 0 && ($cb['until'] > $timenow || empty($cb['until']) || $cb['until'] == "0")) {
		if(empty($cb['until']) || $cb['until'] == "0") {
			...
			echo "<div id='login_response'><h5 class='text-center red'>". $m['you_are_banned'] ."<br>". $cb['reason'] ."<br><br>". $m['block_expires'] ."<br>". date("d M Y", $cb['until']) ." ". $m['at'] ." ". date("G:i", $cb['until']) ."</h5></div>";
		}
	...
	// Check if the user is logged in
		...
		// Check if a Stripe payment is going to be made
			...
			$check_uid = mysqli_query($con,"SELECT * FROM users WHERE id='$uid' AND active='0'");
